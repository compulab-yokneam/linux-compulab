...
static struct gpio_desc *pwdn_gpio = NULL;
static struct gpio_desc *reset_gpio = NULL;

static int gpios_available_1335(void) {
	return (pwdn_gpio != NULL ) && (reset_gpio != NULL);
}
...
static void toggle_gpio(struct gpio_desc *gpio, int val)
{
...
static int ar1335_parse_and_get_gpios(struct device *dev)
{
	...
	pwdn_gpio = devm_gpiod_get_optional(dev, "pwm-gpios", GPIOD_OUT_LOW);
	if (IS_ERR(pwdn_gpio))
		return dev_err_probe(dev, PTR_ERR(pwdn_gpio), "Failed to get gpio 'power'\n");


	reset_gpio = devm_gpiod_get_optional(dev, "rst-gpios", GPIOD_OUT_LOW);
	if (IS_ERR(reset_gpio))
		return dev_err_probe(dev, PTR_ERR(reset_gpio), "Failed to get gpio 'reset'\n");
